...
- **Global news source**: uses Yahoo Finance's own news RSS (`finance.yahoo.com/news/rssindex`)
  rather than a scraped aggregator, since it's official, stable, and matches the rest of the
  app's Yahoo-based data. Swap the `global` URL in `app/api/news/route.ts` for something else
  (Reuters, CNBC, etc.) if you'd prefer a different source.
