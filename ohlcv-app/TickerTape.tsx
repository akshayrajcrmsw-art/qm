...
function badgeText(label: string): string {
  // Short 2-3 char badge from the label, e.g. "Nifty Bank" -> "NB",
  // "S&P 500" -> "SP", "Bitcoin" -> "BI".
  const words = label.replace(/[^a-zA-Z0-9& ]/g, "").split(" ").filter(Boolean);
  if (words.length >= 2) return (words[0][0] + words[1][0]).toUpperCase();
  return label.slice(0, 2).toUpperCase();
}

function AreaSparkline({ values, isUp }: { values: number[]; isUp: boolean }) {
  ...
  const color = isUp ? "#3ddc97" : "#ff5c5c";
...
function TickerChip({ item }: { item: TickerItem }) {
  ...
  return (
    <div className="chip">
      <span className="badge mono" style={{ borderColor: color, color }}>
