...
interface DashboardChartPanelProps {
  ...
  interval: "1wk" | "1d";
  label: string;
  /** Also enables StdDev bands, volume profile, and a light Monte Carlo cone. */
  showEMA: boolean;
...
export default function DashboardChartPanel({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        setVolumeProfile(showEMA ? data.volumeProfile || null : null);
  ...
  // A light Monte Carlo cone on the "full analysis" panel — pure client-side
  // math over data already loaded, same simulation used on the advanced
  // chart page, just with a shorter default projection to keep it readable
  // at this panel's smaller size.
  useEffect(() => {
    if (!showEMA || candles.length === 0) {
      setMonteCarlo(null);
      return;
    }
    setMonteCarlo(runMonteCarloSimulation(candles, { barsAhead: 30, simulations: 300 }));
  }, [showEMA, candles]);
